...
func TestToJSON(t *testing.T) {
	...
	run := func(val string, expected string) {
		_, logs, err := runIntr(intrParams{
			stdlib: map[string]string{"init.sky": fmt.Sprintf("print(to_json(%s))", val)},
		})
		So(err, ShouldBeNil)
		So(logs, ShouldResemble, []string{fmt.Sprintf("[builtin:init.sky:1] %s", expected)})
	}
	...
	Convey("Detects recursive structures", t, func() {
		...
		err := runScript(`
			...
			print(to_json(a))
		`)
		So(err.(*skylark.EvalError).Msg, ShouldEqual, msg)
		...
		// And there can't be recursion involving sets, since Skylark sets require
	...
	Convey("Bad calls", t, func() {
		...
		So(err.(*skylark.EvalError).Msg, ShouldEqual, "to_json: got 0 arguments, want 1")
		err = runScript(`to_json(1, 2, 3)`)
		So(err.(*skylark.EvalError).Msg, ShouldEqual, "to_json: got 3 arguments, want 1")
		err = runScript(`to_json(k=1)`)
		So(err.(*skylark.EvalError).Msg, ShouldEqual, "to_json: unexpected keyword arguments")
	})
}
